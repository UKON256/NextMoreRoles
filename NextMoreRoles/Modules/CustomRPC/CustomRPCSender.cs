...
//TOHからいただきました(主に空き瓶さん)ありがとうございます！
...
namespace NextMoreRoles.Modules.CustomRPC
{
    ...
    public class CustomRpcSender
    {
        ...
        public CustomRpcSender(string name, SendOption sendOption, bool isUnsafe)
        {
            ...
            Logger.Info("RPCSenderで"+$"\"{name}\"をスタックしました", "CustomRPCSender");
        }
        ...
        public CustomRpcSender StartMessage(int targetClientId = -1)
        {
            if (currentState != State.Ready)
            {
                ...
                if (isUnsafe)
                {
                    Logger.Warn(errorMsg, "CustomRPCSender");
                }
                else
                {
        ...
        public void SendMessage()
        {
            ...
            Logger.Info("RPCSenderで"+$"\"{name}\"の送信が終了しました", "CustomRPCSender");
            stream.Recycle();
        }
